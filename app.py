...
with tab6:
    ...
    st.markdown("##### Evaluación clínica basada en síntomas, antecedentes y hallazgos oftalmológicos")
    st.info("💡 Este sistema clasifica el grado de RD según criterios internacionales (ETDRS / AAO). No reemplaza el juicio clínico de un oftalmólogo.")
    ...
    if st.button("🔬 Clasificar Retinopatía", use_container_width=True, type="primary"):
        ...
        st.markdown(f"""
        <div style='background:linear-gradient(135deg,rgba(0,30,50,0.92),rgba(0,60,90,0.88));
                    border:2px solid {color_dx};border-radius:18px;padding:28px 32px;margin-bottom:20px;
                    box-shadow:0 0 35px {color_dx}44;'>
            <h2 style='color:{color_dx};margin:0 0 6px 0;'>{icono_dx} {grado}</h2>
            <p style='color:#FFD166;font-weight:600;font-size:1.05em;margin:0 0 12px 0;'>⚡ {urgencia}</p>
            <p style='color:#B0E8F7;font-size:0.95em;line-height:1.7;margin:0;'>{descripcion}</p>
        </div>
        """, unsafe_allow_html=True)

        # Métricas resumen
        col_m1, col_m2, col_m3 = st.columns(3)
        col_m1.metric("Score diagnóstico", f"{score} pts", "Hallazgos ponderados")
        col_m2.metric("Factores de riesgo", f"{len(factores_riesgo)}", "Antecedentes relevantes")
        col_m3.metric("Estadio", grado.split("(")[-1].replace(")", "") if "(" in grado else "—")
